  ...
  render() {
    return (
      <tr onClick={() => this.setState({clicked: !this.state.clicked})}>
        <td className="fish-name">this.props.fishData.name</td>
        <td>
          <img src={this.props.fishData.image} />
        </td>
        {this.state.clicked ? <td className="fish-description">{this.props.fishData.description}</td> : null}
      </tr>
    )
  }
}
